...
(function ($) {
   $.fn.jsRapCalendar = function (options) {
      ...
      return this.each(function () {
         ...
         let tbody = $('<tbody onclick="GetFecha()">').appendTo(table);
         ...
         this.ShowMonth = function (year, month, del) {
            ...
            for (var n = 1; n <= dim; n++) {
               ...
               if (D3ToStr(this.curYear, this.curMonth, n) < this.opt.minDate)
                  d.addClass('calSelectable'); //d.addClass('calDisable');
            ...
            $('.calSelectable', this).bind({
               click: function (e) {
                  ...
                  if (base.opt.onClick)
                     ...
                     fecha_select = base.selD + "-" + base.selM + "-" + base.selY;
                  fechaGet = fecha_select.split("-")

                  var mes = parseInt(fechaGet[1]) + 1;
               
               if(fechaGet[0]<9){
                  fecha_select = fechaGet[2] + "-" + mes.toString() + "-0" + fechaGet[0]
                 }else{
                  fecha_select = fechaGet[2] + "-" + mes.toString() + "-" + fechaGet[0]
                 }

                  $("#fechaSelect").val(fecha_select);
                  // fetch('/info')
